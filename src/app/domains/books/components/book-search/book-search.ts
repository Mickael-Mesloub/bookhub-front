...
export class BookSearch {
  ...
  protected categoryFilters: WritableSignal<
  ...
  protected availabilityFilters: WritableSignal<
    TypeCreatedFromEnumWithALL<typeof AvailabilityCategory>
  > = signal(this.buildInitialFiltersAndSorts(AvailabilityCategory, 'ALL'));

  protected sorts: WritableSignal<TypeCreatedFromEnumWithALL<typeof SortCategory>> = signal(
  ...
  readonly categoryFiltersList = computed(() =>
    Object.entries(this.categoryFilters()).map(([key, value]) => ({
      ...
      value,
      label:
        BookCategoryOptionLabels[key as keyof typeof BookCategoryOptionLabels] ??
        'Toutes les catégories', // pour pouvoir afficher le libellé
    })),
  );
  readonly availabilityFiltersList = computed(() =>
    Object.entries(this.availabilityFilters()).map(([key, value]) => ({
      key: key as keyof TypeCreatedFromEnumWithALL<typeof AvailabilityCategory>,
      value,
      label:
        AvailabilityCategory[key as keyof typeof AvailabilityCategory] ??
        'Toutes les disponibilités', // pour pouvoir afficher le libellé
    })),
  );
  readonly sortList = computed(
    () =>
      Object.entries(this.sorts()).map(([key, value]) => ({
